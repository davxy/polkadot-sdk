...
fn generate_epoch_tickets(
	...
	for (authority_idx, authority_id) in epoch.authorities.iter().enumerate() {
		...
		trace!(target: LOG_TARGET, "Generating ring prover key...");
...
where
	...
	E: Environment<B, Error = ER> + Send + Sync,
...
///
/// A new epoch announcement trigger the begin of the generation of tickets for the next epoch.
///
	...
	SC: SelectChain<B> + 'static,
{
	...
	while let Some(notification) = notifications.next().await {
		...
		trace!(target: LOG_TARGET, "Announcement: {:x?}", epoch_desc);
